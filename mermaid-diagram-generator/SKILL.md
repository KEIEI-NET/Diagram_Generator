...
function validateMermaidSyntax(mermaidCode) {
  const checks = {
    syntaxValid: checkSyntax(mermaidCode),
    elementCount: countElements(mermaidCode),
    lineLength: checkLineLength(mermaidCode),
    specialChars: checkSpecialCharacters(mermaidCode)
  };
  
  return {
    isValid: Object.values(checks).every(c => c.passed),
    checks
  };
}
```
